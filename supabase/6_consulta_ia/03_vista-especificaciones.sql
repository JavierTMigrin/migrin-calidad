...
clientes AS (
  -- cliente = nombre legible (campo "nombre" dentro del spec), no la clave
  -- cruda del JSON (evita mismatches como 'LIRQUEN' vs "Vidrios Lirquen").
  SELECT b.producto_key,
         COALESCE(NULLIF(kv.value->>'nombre',''), kv.key) AS cliente,
         kv.value AS spec
...
COMMENT ON COLUMN v_ia_especificaciones.cliente IS 'Nombre legible del cliente/especificacion (ej. Vidrios Lirquen, Cristalerias Chile). Para productos con una sola EETT, es el nombre del producto.';
